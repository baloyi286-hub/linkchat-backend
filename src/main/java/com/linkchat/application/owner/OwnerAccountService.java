...
public class OwnerAccountService {
    ...
    @Transactional(readOnly=true) public Object inbox(String authSubject){Account owner=requireOwner(authSubject);List<Map<String,Object>> all=new java.util.ArrayList<>();chat.inbox(owner.getInviteCode()).stream().filter(item->!Boolean.TRUE.equals(item.get("hidden"))).forEach(item->{Map<String,Object> copy=new java.util.LinkedHashMap<>(item);copy.put("role","OWNER");copy.put("otherName",item.get("visitorName"));all.add(copy);});all.addAll(chat.visitorInbox(owner.getId()));all.sort((a,b)->String.valueOf(b.get("createdAt")).compareTo(String.valueOf(a.get("createdAt"))));return all;}
